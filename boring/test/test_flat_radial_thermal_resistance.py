...
class TestFlatRadialResistance(unittest.TestCase):

    def setUp(self):
        p2 = self.prob = Problem(model=Group())
        p2.model.add_subsystem('cond_thermal', subsys=RadialThermalResistance(num_nodes=40, geom='flat'), promotes=['*'])

        p2.setup(force_alloc_complex=True)
        p2.run_model()

    def test_flat_cond_outputs(self):

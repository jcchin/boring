...
def opt_cool_plots(filename, x):
    ...
    for z,f in enumerate(filename):
        ...
        #print(zones)
        labels = [r'h=10 $\frac{W}{m^2K}$','h=100','h=250']

        soff = 0.076
        ioff = 30
        ax[1].plot(cell_dens[z],cell_dens[z]*(slope[z]+soff)+intercept[z]-ioff, label=labels[z]) # *0.412+80
        ax[1].set_ylabel('Wh/kg pack')
        ax[1].set_xlabel('Wh/kg cell')

        ax[0].plot(cell_dens[z],cell_dens[z]*slope2[z]+intercept2[z])
...
if __name__ == '__main__':
    ...
    opt_cool_plots(['../metamodel/test_10.csv','../metamodel/test_100.csv','../metamodel/test_250.csv'],30)

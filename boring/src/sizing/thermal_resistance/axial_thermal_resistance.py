...
class AxialThermalResistance(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('L_flux', 4*np.ones(nn), units='m', desc='flux length') # Effective, not Adiabatic!
        self.add_input('L_adiabatic', 4*np.ones(nn), units='m', desc='Effective Length') # Effective, not Adiabatic!
    ...
    def setup_partials(self):
        ...
        self.declare_partials('R_aw', ['L_flux', 'L_adiabatic', 'A_w', 'k_w'], rows=ar, cols=ar, val=0.)
        self.declare_partials('R_awk', ['L_flux', 'L_adiabatic', 'A_wk', 'epsilon', 'k_w', 'k_l'], rows=ar, cols=ar, val=0)

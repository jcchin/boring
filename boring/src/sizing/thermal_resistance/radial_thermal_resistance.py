...
class RadialThermalResistance(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('A_inter', val=1.0*np.ones(nn), units='m**2', desc='area of wick/vapor interface of the condenser')

        self.add_output('h_inter', val=1.0*np.ones(nn), units='W/(m**2/K)', desc='HTC of wick/vapor interface of the condenser')
        ...
        self.add_output('R_inter', val=1.0*np.ones(nn), units='K/W', desc='thermal resistance of wick/vapor interface of the condenser')
    ...
    def setup_partials(self):
        ...
        self.declare_partials('R_w', ['D_od', 'r_i', 'k_w', 'L_cond'], rows=ar, cols=ar)
        self.declare_partials('R_wk', ['D_v', 'r_i', 'k_wk', 'L_cond'], rows=ar, cols=ar)
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['R_w'] = np.log((D_od/2)/(r_i))/(2*np.pi*k_w*L_cond) 
        outputs['R_wk'] = np.log((r_i)/(D_v/2))/(2*np.pi*k_wk*L_cond) 
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials['R_w', 'D_od'] = 1/(D_od*(2*np.pi*k_w*L_cond)) 
        partials['R_w', 'r_i'] = -1/(2*np.pi*k_w*L_cond)
        partials['R_w', 'k_w'] = -1 * np.log((D_od/2)/(r_i))/(2*np.pi*k_w**2*L_cond) 
        partials['R_w', 'L_cond'] = -1 * np.log((D_od/2)/(r_i))/(2*np.pi*k_w*L_cond**2) 

        partials['R_wk', 'r_i'] = 1/(r_i*(2*np.pi*k_wk*L_cond)) 
        partials['R_wk', 'D_v'] = -1/(D_v*(2*np.pi*k_wk*L_cond)) 
        partials['R_wk', 'k_wk'] = -1 * np.log((r_i)/(D_v/2))/(2*np.pi*k_wk**2*L_cond)
        partials['R_wk', 'L_cond'] = -1 * np.log((r_i)/(D_v/2))/(2*np.pi*k_wk*L_cond**2)

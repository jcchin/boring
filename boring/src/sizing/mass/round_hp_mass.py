...
class roundHPmass(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('XS:D_od',val=0.03*np.ones(nn), desc='heatpipe outer diameter', units='m')
        self.add_input('XS:D_v',val=0.03*np.ones(nn), desc='vapor outer diameter', units='m')
        self.add_input('L_heatpipe',val=0.3*np.ones(nn), desc='Total length of heatpipe', units='m')
        self.add_input('XS:t_w', val=.0005*np.ones(nn), desc='wall thickness of heatpipe', units='m')
        self.add_input('XS:t_wk', val=0.0005*np.ones(nn), desc='wick thickness', units='m')
    ...
    def compute_partials(self,i,J):
        ...
        J['mass_wick', 'XS:t_wk'] =L_heatpipe*cu_density*fill_wk*  np.pi*2*( D_v/2 + t_wk )
        J['mass_wick', 'XS:D_v'] =L_heatpipe*cu_density*fill_wk*   np.pi*( (D_v/2+t_wk) - (D_v/2) )

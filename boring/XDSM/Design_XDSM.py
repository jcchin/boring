...
x.add_input('Optimizer','T_{limit}')
x.connect('Optimizer','heat_pipe', ['D_{hp}, (W_{hp})','t_{wall}','t_{wick}','epsilon_{hp}','L_{hp}'])
x.connect('Optimizer','transient', ['D_{hp}, (W_{hp})','t_{wall}','t_{wick}','epsilon_{hp}','L_{hp}'])
x.connect('Optimizer','PCM', ['porosity_{pcm}'])

# Case
x.connect('case','pack',['mass_{case}','t_{case}'])
#x.add_output('case', ['mass_{battery}'], side='right')

# Battery
x.add_input('battery',['L_{cell}', 'W_{cell}', 'H_{cell}'])
...
x.connect('PCM', 'pack',['mass_{pcm}','t_{pcm}'])
